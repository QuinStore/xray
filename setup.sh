...
clear;clear;clear
  ...
  # // Banner
echo -e "${YELLOW}----------------------------------------------------------${NC}"
echo -e "  Welcome To SDC TUNNELING SCRIPT ${YELLOW}(${NC}${green} Stable Edition ${NC}${YELLOW})${NC}"
...
#Repo git
repo="https://raw.githubusercontent.com/QuinStore/";
#########################
# USERNAME
rm -f /usr/bin/user
username=$(curl ${repo}reseller/main/user/izin | grep $MYIP | awk '{print $2}')
echo "$username" >/usr/bin/izin
expx=$(curl ${repo}reseller/main/izin | grep $MYIP | awk '{print $3}')
echo "$expx" >/usr/bin/e
# DETAIL ORDER
username=$(cat /usr/bin/izin)
oid=$(cat /usr/bin/ver)
exp=$(cat /usr/bin/e)
clear
# CERTIFICATE STATUS
...
clear
# REPO    
    instalasi="https://raw.githubusercontent.com/QuinStore/xray/main/"
...
print_install "Memasang Password SSH"
    wget -O /etc/pam.d/common-password "${instalasi}config/password"
...
#cd /usr/local/sbin/
#sed -i 's/\r//' quota
wget https://raw.githubusercontent.com/QuinStore/xray/main/limit/limit.sh && chmod +x limit.sh && ./limit.sh
...
port 587
auth on
user tamiminang@gmail.com
from tamiminang@gmail.com
password denggol123
logfile ~/.msmtp.log
EOF
