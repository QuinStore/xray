...
Name=$(curl https://raw.githubusercontent.com/QuinStore/reseller/main/izin | grep $MYIP | awk '{print $2}')
...
botBackup() {
	...
	sleep 1
    echo -e "[ ${green}INFO${NC} ] â€¢ VPS Data Backup... "
    sleep 1
    echo -e "[ ${green}INFO${NC} ] â€¢ Directory Created... "
    mkdir /root/backup &>/dev/null
    sleep 1
    echo -e "[ ${green}INFO${NC} ] â€¢ VPS Data Backup Start Now... "
    echo -e "[ ${green}INFO${NC} ] â€¢ Please Wait , Backup In Process Now... "
    sleep 1
	cp /etc/passwd backup/ &>/dev/null
	echo -e "[ ${green}INFO${NC} ] â€¢ Backup passwd data..."
	sleep 1
	cp /etc/group backup/ &>/dev/null
	echo -e "[ ${green}INFO${NC} ] â€¢ Backup group data..."
	sleep 1
	cp /etc/shadow backup/ &>/dev/null
	echo -e "[ ${green}INFO${NC} ] â€¢ Backup shadow data..."
	sleep 1
	cp /etc/gshadow backup/ &>/dev/null
	echo -e "[ ${green}INFO${NC} ] â€¢ Backup gshadow data..."
	sleep 1
    ...
    echo -e "[ ${green}INFO${NC} ] â€¢ Sending Via Bot... "
    ...
    echo -e "[ ${green}INFO${NC} ] â€¢ Completed... "
...
restoreBot() {
    ...
    echo -e "[ ${green}INFO${NC} ] â€¢ Restore Data..."
	read -rp "Password File: "  InputPass
	echo -e "[ ${green}INFO${NC} ] â€¢ Getting your data..."
    ...
    rm -rf /root/backup
    rm -f backup.zip
    echo -e "[ ${green}INFO${NC} ] â€¢ Done Restore... "
    read -n 1 -s -r -p "Press any key to back on menu"
    menu
}
...
botbckpBot_menu() {
	...
	echo -e " ${Blue}└──────────────────────────────────────────┘${NC}"
    echo -e " ${Blue}┌──────────────────────────────────────────┐${NC}"
    echo -e " ${Blue}â”‚$NC ${green} VPS Data Backup By PT"
    echo -e " ${Blue}â”‚$NC"
